...
try {
  const fs = require("fs");
  const logoPath = path.join(hotelDir, "fiscal", "assets", "logo_rks_mf.png");
  const { generateRksMfLogo } = require(path.join(
    hotelDir,
    "fiscal",
    "assets",
    "generate-rks-mf-logo.js"
  ));
  if (!fs.existsSync(logoPath)) {
    generateRksMfLogo(logoPath);
    console.log("[pre-build] ✓ Logo fiskale RKS/MF u gjenerua.\n");
  }
} catch (e) {
  console.warn("[pre-build] Logo RKS/MF:", e.message);
}
