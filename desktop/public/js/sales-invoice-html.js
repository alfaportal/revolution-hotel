/* Fatura A4 — HTML preview + print (desktop HOTEL) */
(function (global) {
  function esc(s) {
    return String(s ?? "")
      .replace(/&/g, "&amp;")
...
@media print {
  ...
  function buildInvoiceHtml(invoice) {
    ...
    const totals = invoice.totals || computeTotals(lines, vat);
    ...
    const rows = lines.map((ln, i) => {
      ...
      return `<tr>
        ...
        <td class="num">${fmtMoney(lt, currency)}</td>
      </tr>`;
    }).join("");
