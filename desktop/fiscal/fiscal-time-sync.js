 ...
 * Offline: ora lokale e Windows (Date.now) ose monotonic + gjendje e mbrojtur (HMAC).
...
const { isAtkCommunicationForbidden } = require("./fiscal-atk-guard");
  ...
  return new Promise((resolve) => {
    ...
    const req = mod.request(

      {
        ...
        lookup: isAtkHost(hostname) ? atkDnsLookup : undefined,
...
async function fetchServerTimeMs() {
  ...
  if (isAtkCommunicationForbidden()) {
    ...
    hosts = hosts.filter((h) => !isAtkHost(String(h || "").trim()));
