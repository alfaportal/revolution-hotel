...
function generateFiscalReceipt(orderData, fiscalData) {
  ...
  if (fiscal.chain_current_hash || fiscal.chain_previous_hash) {
    lines.push(divider("-", w));
    lines.push(`^C${pad(t("hash_chain_title"), w, "center")}`);
    lines.push(padLine(t("previous_hash"), formatHashShort(fiscal.chain_previous_hash, 20), w));
    lines.push(padLine(t("current_hash"), formatHashShort(fiscal.chain_current_hash, 20), w));
    lines.push(
      padLine(
        t("integrity_check"),
