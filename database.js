...
function getTableIdByNumber(tableNumber) {
  ...
  const row = sqlite.prepare("SELECT id FROM tables WHERE number = ?").get(n);
  return row ? Number(row.id) : null;
}

/** Rresht tavolinë → format i vjetër reservations_local (për reservation-sync). */
function mapTableReservationStorageRow(row) {
  if (!row) return null;
  const date = String(row.check_in_date || row.start_time || "").slice(0, 10);
  const time = String(row.slot_time || row.start_time || "").slice(11, 16) || "19:00";
  return {
    ...row,
    id: row.legacy_local_id || String(row.id),
    internal_id: row.id,
    customer_name: row.customer_name || row.guest_name || "",
    customer_phone: row.customer_phone || row.phone || "",
    date,
    time,
    guests: Number(row.guest_count ?? row.persons) || 2,
    table_number: Number(row.table_number) || 0,
  };
}

function findTableReservationStorageRow(id) {
  const key = String(id || "").trim();
  if (!key) return null;
  const row = sqlite.prepare(`
    SELECT * FROM reservations
    WHERE reservation_type = 'table'
      AND (legacy_local_id = ? OR cloud_id = ? OR CAST(id AS TEXT) = ?)
    LIMIT 1
  `).get(key, key, key);
  return row ? mapTableReservationStorageRow(row) : null;
}

function upsertReservationLocal(row) {
  const tableNum = Number(row.table_number);
  const targetId = getTableIdByNumber(tableNum);
  const date = String(row.date).slice(0, 10);
  const slot = String(row.time || "19:00").slice(0, 5);
  const startTime = `${date} ${slot}:00`;
  const endTime = `${date} ${slot}:00`;
  ...
  const existing = sqlite.prepare(`
    ...
    WHERE reservation_type = 'table'
      AND (legacy_local_id = ? OR cloud_id = ? OR CAST(id AS TEXT) = ?)
    LIMIT 1
  `).get(legacyId, row.cloud_id || legacyId, legacyId);
  ...
  if (existing) {
    sqlite.prepare(`
      ...
      WHERE id = ?
    `).run(...payload.slice(1), existing.id);
    return existing.id;
  }

  const info = sqlite.prepare(`
    INSERT INTO reservations (
      reservation_type, target_id, customer_name, customer_phone,
      start_time, end_time, guest_count, status,
      slot_time, table_number, cloud_id, sync_status, conflict_message,
      pending_status, legacy_local_id, notes
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(...payload);
...
function getLocalReservation(id) {
  return findTableReservationStorageRow(id);
}
...
function listLocalReservations(query = {}) {
  ...
  const rows = sqlite.prepare(`
    SELECT * FROM reservations
    WHERE reservation_type = 'table' AND ${f.sql.replace(/\bdate\b/g, "date(start_time)")}
    ORDER BY start_time ASC, slot_time ASC
  `).all(...f.params);
  return rows.map(mapTableReservationStorageRow);
}
...
function listPendingReservationSync() {
  return sqlite.prepare(`
    SELECT * FROM reservations
    WHERE reservation_type = 'table'
      AND (sync_status = 'pending' OR (pending_status IS NOT NULL AND pending_status != '' AND cloud_id IS NOT NULL))
    ORDER BY created_at ASC
  `).all().map(mapTableReservationStorageRow);
}

function updateLocalReservationSync(id, fields) {
  const row = findTableReservationStorageRow(id);
  if (!row) return;
  const sets = [];
  const vals = [];
  const map = {
  ...
  for (const [k, v] of Object.entries(fields)) {
    if (k === "date" && v) {
      const slot = String(fields.time || row.time || "19:00").slice(0, 5);
      const d = String(v).slice(0, 10);
      sets.push("start_time = ?", "end_time = ?");
      vals.push(`${d} ${slot}:00`, `${d} ${slot}:00`);
      continue;
    }
    if (k === "time" && v && !fields.date) {
      const d = String(row.date || row.start_time || "").slice(0, 10);
      const slot = String(v).slice(0, 5);
      sets.push("slot_time = ?", "start_time = ?", "end_time = ?");
      vals.push(slot, `${d} ${slot}:00`, `${d} ${slot}:00`);
      continue;
    }
    const col = map[k];
    if (!col) continue;
  ...
  sqlite.prepare(`UPDATE reservations SET ${sets.join(", ")} WHERE id = ?`).run(
    ...vals,
    row.internal_id,
  );
}
...
function addMenuItem({
  name, category, price, vat_category, stock_qty, low_stock_threshold, barcode, auto_vat, operator_name,
} = {}) {
  ...
  if (stock_qty != null && Number.isFinite(Number(stock_qty))) {
    setMenuItemStockAbsolute(
      id,
      Math.max(0, Number(stock_qty)),
      "Stoku fillestar — produkt i ri (menuja)",
      operator_name || "",
    );
  }
...
function updateMenuItem(id, {
  name, category, price, low_stock_threshold, vat_category, stock_qty, barcode, operator_name,
} = {}) {
  ...
  if (!fields.length && stockTarget == null) return;
  if (fields.length) {
    vals.push(id);
    sqlite.prepare(`UPDATE menu_items SET ${fields.join(", ")} WHERE id = ?`).run(...vals);
  }
  if (stockTarget != null) {
    setMenuItemStockAbsolute(id, stockTarget, "", operator_name || "");
  }
}
...
function resolveGuestRoomTarget(roomNumber) {
  ...
  const reservation = sqlite.prepare(`
    SELECT id FROM reservations
    WHERE room_id = ?
      AND (reservation_type IS NULL OR reservation_type = 'room')
...
function getRoomReservationById(id) {
  const row = sqlite.prepare(`
    ...
    WHERE rv.id = ?
      AND (rv.reservation_type IS NULL OR rv.reservation_type = 'room')
  `).get(Number(id)) || null;
  if (!row) return null;
...
function listRoomReservations({ status, from, to, on_date, limit } = {}) {
  ...
  clauses.push("(rv.reservation_type IS NULL OR rv.reservation_type = 'room')");
  if (on_date) {
...
/** Rezervime të unifikuara (dhoma + tavolinë) për kalendar / listë të përbashkët. */
function listUnifiedReservations({ from, to, types, status, limit } = {}) {
  const fromDate = from ? parseHotelDate(from) : hotelTodayLocalYmd();
  const toDate = to ? parseHotelDate(to) : fromDate;
  const allowedTypes = Array.isArray(types) && types.length
    ? types.map((t) => String(t).toLowerCase()).filter((t) => t === "room" || t === "table")
    : ["room", "table"];
  const params = [...allowedTypes, toDate, fromDate];

  let statusSql = "";
  const st = String(status || "").trim().toLowerCase();
  if (st === "active") {
    statusSql = " AND rv.status IN ('pending', 'confirmed')";
  } else if (st && st !== "all") {
    statusSql = " AND rv.status = ?";
    params.push(st);
  }

  const typePlaceholders = allowedTypes.map(() => "?").join(", ");
  const lim = Math.min(500, Math.max(1, Number(limit) || 300));
  const rows = sqlite.prepare(`
    SELECT
      rv.*,
      r.room_number,
    ...
    FROM reservations rv
    LEFT JOIN rooms r ON COALESCE(rv.reservation_type, 'room') = 'room' AND r.id = COALESCE(rv.target_id, rv.room_id)
    LEFT JOIN tables t ON rv.reservation_type = 'table' AND t.id = rv.target_id
    WHERE COALESCE(rv.reservation_type, 'room') IN (${typePlaceholders})
      AND date(COALESCE(rv.start_time, rv.check_in_date)) <= ?
      AND date(COALESCE(rv.end_time, rv.check_out_date, rv.start_time, rv.check_in_date)) >= ?
      ${statusSql}
    ORDER BY COALESCE(rv.start_time, rv.check_in_date || ' 00:00:00') ASC, rv.id ASC
    LIMIT ${lim}
  `).all(...params);

  return rows.map((row) => {
    const type = row.reservation_type || "room";
    if (type === "table") {
      const mapped = mapTableReservationStorageRow(row);
      return {
        ...mapped,
        reservation_type: "table",
        target_id: row.target_id,
        label: `T${mapped.table_number} · ${mapped.time}`,
        title: mapped.customer_name,
      };
    }
    return {
      ...row,
      reservation_type: "room",
      target_id: row.target_id || row.room_id,
      customer_name: row.customer_name || row.guest_name,
      start_time: row.start_time || `${row.check_in_date} 14:00:00`,
      end_time: row.end_time || `${row.check_out_date} 11:00:00`,
      label: `Dh. ${row.room_number || "?"} · ${row.check_in_date}`,
      title: row.guest_name || row.customer_name,
      services: listReservationServices(row.id),
    };
  });
}
...
function createRoomReservation({
  ...
  const r = sqlite.prepare(`
    INSERT INTO reservations (
      reservation_type, target_id, customer_name, customer_phone, start_time, end_time, guest_count,
      room_id, guest_name, phone, email, check_in_date, check_out_date, persons, deposit, notes, status, sync_status
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'local')
  `).run(
    "room",
    Number(room_id),
...
function updateRoomReservation(id, {
  ...
  if (st === "cancelled") {
    sqlite.prepare(`
      UPDATE reservations
      SET reservation_type = 'room', target_id = ?, customer_name = ?, customer_phone = ?,
          start_time = ?, end_time = ?, guest_count = ?,
          room_id = ?, guest_name = ?, phone = ?, email = ?, check_in_date = ?, check_out_date = ?,
          persons = ?, deposit = ?, notes = ?, status = ?
      WHERE id = ? AND (reservation_type IS NULL OR reservation_type = 'room')
    `    ).run(
      roomId, name, phoneVal, `${inDate} 14:00:00`, `${outDate} 11:00:00`, Math.trunc(personsNum),
...
/** Regjistron ndryshim manual stoku (±) — për reconciliacion, jo kontabilist. */
function logManualStockAdjustment(menuItemId, qty, note = "", operatorName = "") {
  const id = Number(menuItemId);
  const q = Number(qty);
  if (!id || !Number.isFinite(q) || Math.abs(q) < 0.0001) return null;
  const r = sqlite.prepare(`
    INSERT INTO stock_manual_adjustments (menu_item_id, quantity, note, operator_name)
    VALUES (?, ?, ?, ?)
  `).run(
    id,
    q,
    String(note || "").slice(0, 240),
    String(operatorName || "").slice(0, 120),
  );
  return Number(r.lastInsertRowid);
}

/** Kur stoku ndryshohet direkt (menuja), regjistro deltën për reconcileStock. */
function logManualStockSetDelta(menuItemId, beforeQty, afterQty, note = "", operatorName = "") {
  const before = Number(beforeQty) || 0;
  const after = Number(afterQty) || 0;
  const delta = after - before;
  if (Math.abs(delta) < 0.0001) return null;
  const defaultNote = delta > 0 ? "Rritje stoku nga menuja" : "Ulje stoku nga menuja";
  return logManualStockAdjustment(menuItemId, delta, note || defaultNote, operatorName);
}

function aggregateManualStockQtyByMenuItem({ from, to } = {}) {
  const rows = from && to
    ? sqlite.prepare(`
      SELECT menu_item_id, COALESCE(SUM(quantity), 0) AS qty
...
/**
 * Charge në dhomë që zbriti stokun (RS / POS / RS-QR) — JO (T#) që shkon te daily_log.
 */
function isRoomChargeStockDecrement(description) {
  const desc = String(description || "").trim();
  if (!desc) return false;
  if (/\(\s*T\d+\s*\)\s*$/i.test(desc)) return false;
  return /\(\s*(RS|POS|RS-QR)\s*\)\s*$/i.test(desc);
}

function parseRoomChargeLineQty(description) {
  const desc = String(description || "").trim();
  const m = desc.match(/^(\d+(?:\.\d+)?)\s*[×x]\s/i);
  return m ? Number(m[1]) || 1 : 1;
}

/** Shitje nga room_charges që kanë zbritur stokun (Room Service etj.). */
function aggregateSalesQtyFromRoomCharges({ from, to } = {}) {
  const rows = from && to
    ? sqlite.prepare(`
      SELECT menu_item_id, description
      FROM room_charges
  ...
  const map = new Map();
  for (const row of rows) {
    if (!isRoomChargeStockDecrement(row.description)) continue;
    const menuItemId = Number(row.menu_item_id);
    if (!menuItemId) continue;
    const qty = parseRoomChargeLineQty(row.description);
    if (!(qty > 0)) continue;
...
function mergeQtyMaps(...maps) {
  ...
  for (const map of maps) {
    for (const [id, qty] of map) {
      out.set(id, (out.get(id) || 0) + (Number(qty) || 0));
    }
  }
  return out;
}

/** Shitje totale për stok: daily_log (tavolinë) + room service / POS në dhomë. */
function aggregateSalesQtyForStock({ from, to } = {}) {
  return mergeQtyMaps(
    aggregateSalesQtyFromDailyLog({ from, to }),
    aggregateSalesQtyFromRoomCharges({ from, to }),
  );
}
...
function computeCogsForPeriod({ from, to } = {}) {
  ...
  const menu = getMenuItems(false);
  const byId = new Map(menu.map((m) => [Number(m.id), m]));
  const sales = aggregateSalesQtyForStock({ from: fromDate, to: toDate });
  const lines = [];
  let total = 0;
...
/**
 * Reconciliacion stoku:
 * fillestar + blerjet (goods) + ndryshime manuale (±) − shitjet (daily_log + room service) vs stock_qty.
 */
function buildMenuItemReconcileRow(m, purchases, salesDaily, salesRoom, manualAdds) {
  const id = Number(m.id);
  const opening = getMenuItemOpeningStock(id);
  const purchased = purchases.get(id) || 0;
  const soldDaily = salesDaily.get(id) || 0;
  const soldRoom = salesRoom.get(id) || 0;
  const soldMerged = soldDaily + soldRoom;
  const manualAdded = manualAdds.get(id) || 0;
  const expected = opening + purchased + manualAdded - soldMerged;
  ...
  return {
    ...
    manual_added: roundReportMoney(manualAdded),
    sold: roundReportMoney(soldMerged),
    sold_daily_log: roundReportMoney(soldDaily),
    sold_room_service: roundReportMoney(soldRoom),
...
function getMenuItemStockQty(menuItemId) {
  const row = sqlite.prepare("SELECT COALESCE(stock_qty, 0) AS q FROM menu_items WHERE id = ?").get(
    Number(menuItemId),
  );
  return row ? Number(row.q) || 0 : 0;
}

function computeMenuItemReconcileState(menuItemId, maps) {
  const id = Number(menuItemId);
  const m = getMenuItems(false).find((x) => Number(x.id) === id)
    || sqlite.prepare(
      "SELECT id, name, category, COALESCE(stock_qty,0) AS stock_qty FROM menu_items WHERE id = ?",
    ).get(id);
  if (!m) return null;
  const purchases = maps?.purchases ?? aggregateGoodsPurchaseQtyByMenuItem();
  const salesDaily = maps?.salesDaily ?? aggregateSalesQtyFromDailyLog();
  const salesRoom = maps?.salesRoom ?? aggregateSalesQtyFromRoomCharges();
  const manualAdds = maps?.manualAdds ?? aggregateManualStockQtyByMenuItem();
  return buildMenuItemReconcileRow(m, purchases, salesDaily, salesRoom, manualAdds);
}

function assertMenuItemStockBalanced(menuItemId) {
  const row = computeMenuItemReconcileState(menuItemId);
  if (!row) throw new Error("Produkti nuk u gjet");
  if (row.has_deviation) {
    throw new Error(
      `Stoku «${row.name}» nuk përputhet me historinë (devijim ${row.difference}). Kontaktoni support.`,
    );
  }
  return row;
}

/** Vendos stokun absolut — regjistron deltën manuale dhe verifikon devijim 0. */
function setMenuItemStockAbsolute(menuItemId, targetQty, note = "", operatorName = "", opts = {}) {
  const id = Number(menuItemId);
  const target = Math.max(0, Number(targetQty));
  if (!id || !Number.isFinite(target)) throw new Error("Stoku duhet të jetë 0 ose më shumë.");
  if (!sqlite.prepare("SELECT id FROM menu_items WHERE id = ?").get(id)) {
    throw new Error("Produkti nuk u gjet");
  }
  const before = getMenuItemStockQty(id);
  const apply = () => {
    sqlite.prepare("UPDATE menu_items SET stock_qty = ? WHERE id = ?").run(target, id);
    logManualStockSetDelta(id, before, target, note, operatorName);
    console.log(`[stock] set productId=${id} stockBefore=${before} stockAfter=${target}`);
  };
  if (opts.inTransaction) apply();
  else sqlite.transaction(apply)();
  if (!opts.skipAssert) assertMenuItemStockBalanced(id);
  return { id, stockBefore: before, stock_qty: target };
}

/** Shtim/ulje relative — e njëjta gjurmim si menuja (+ verifikim devijim 0). */
function adjustMenuItemStockByDelta(menuItemId, delta, note = "", operatorName = "") {
  const id = Number(menuItemId);
  const d = Number(delta);
  if (!id || !Number.isFinite(d) || Math.abs(d) < 0.0001) {
    throw new Error("Produkt ose sasi e pavlefshme për stok.");
  }
  const row = sqlite.prepare("SELECT name FROM menu_items WHERE id = ?").get(id);
  if (!row) throw new Error("Produkti nuk u gjet");
  const before = getMenuItemStockQty(id);
  const target = Math.max(0, before + d);
  const result = setMenuItemStockAbsolute(id, target, note || "Ndryshim manual stoku", operatorName);
  return { ...result, name: row.name };
}

/** Një herë: korrigjon devijimet e vjetra (para menu-tracking) me hyrje në histori. */
function baselineStockReconcileDriftsOnce() {
  if (getSetting("stock_reconcile_baseline_v1")) {
    return { skipped: true, healed: 0, mismatch_count: reconcileStock().mismatch_count };
  }
  const rep = reconcileStock();
  let healed = 0;
  sqlite.transaction(() => {
    for (const item of rep.deviations) {
      const diff = Number(item.difference) || 0;
      if (Math.abs(diff) < 0.0001) continue;
      logManualStockAdjustment(
        item.menu_item_id,
        diff,
        "Baseline — korrigjim stoku pa gjurmim (para menu-tracking)",
        "system",
      );
      healed += 1;
    }
    sqlite.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)").run(
      "stock_reconcile_baseline_v1",
      new Date().toISOString(),
    );
  })();
  const after = reconcileStock();
  if (after.mismatch_count > 0) {
    console.warn(`[stock] baseline reconcile: ${healed} healed, ${after.mismatch_count} mismatch mbeten`);
  } else if (healed > 0) {
    console.log(`[stock] baseline reconcile OK — ${healed} artikuj u korrigjuan`);
  }
  return { healed, mismatch_count: after.mismatch_count };
}

function reconcileStock() {
  const menu = getMenuItems(false);
  const purchases = aggregateGoodsPurchaseQtyByMenuItem();
  const salesDaily = aggregateSalesQtyFromDailyLog();
  const salesRoom = aggregateSalesQtyFromRoomCharges();
  const manualAdds = aggregateManualStockQtyByMenuItem();
  const items = [];
  let mismatchCount = 0;

  for (const m of menu) {
    const row = buildMenuItemReconcileRow(m, purchases, salesDaily, salesRoom, manualAdds);
    if (row.has_deviation) mismatchCount += 1;
    items.push(row);
  }
  ...
  return {
    ...
    formula:
      "stoku fillestar + blerjet (goods) + ndryshime manuale (±) − shitjet (ditari + room service)",
...
function getVersionInfo() {
  ...
  setHotelServiceCategoryPhoto,
    ...
    adjustMenuItemStockByDelta,
    setMenuItemStockAbsolute,
    assertMenuItemStockBalanced,
    baselineStockReconcileDriftsOnce,
    logManualStockAdjustment,
    logManualStockSetDelta,
