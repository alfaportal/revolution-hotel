...
function migrateReservationsUnifiedSchema(sqlRun, sqlAll, backupCtx) {
  ...
  sqlRun(`CREATE TABLE reservations_unified (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    reservation_type TEXT NOT NULL DEFAULT 'room',
    target_id INTEGER,
    user_id INTEGER,
  ...
  const oldNames = new Set(cols.map((c) => c.name));
  if (oldNames.has("room_id")) {
    sqlRun(`
      INSERT INTO reservations_unified (
        ...
        persons, deposit, notes, sync_status, created_at, updated_at
      )
      SELECT
        id,
        'room',
        ...
        COALESCE(guest_name, ''),
        COALESCE(phone, ''),
        COALESCE(check_in_date, '') || ' 14:00:00',
        COALESCE(check_out_date, '') || ' 11:00:00',
        COALESCE(persons, 1),
  ...
  if (hasLocal) {
    const localRows = sqlAll("SELECT * FROM reservations_local");
    for (const row of localRows) {
      const tableNum = Number(row.table_number);
      let targetId = null;
      if (tableNum > 0) {
        const t = sqlAll("SELECT id FROM tables WHERE number = ?", [tableNum])[0];
        if (t) targetId = Number(t.id);
      }
      ...
      sqlRun(
        `INSERT INTO reservations_unified (
          ...
          pending_status, legacy_local_id, notes, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          "table",
          targetId,
  ...
  sqlRun("DROP TABLE reservations");
  sqlRun("ALTER TABLE reservations_unified RENAME TO reservations");
  sqlRun("CREATE INDEX IF NOT EXISTS idx_reservations_type ON reservations(reservation_type)");
  sqlRun("CREATE INDEX IF NOT EXISTS idx_reservations_target ON reservations(target_id)");
  sqlRun("CREATE INDEX IF NOT EXISTS idx_reservations_start ON reservations(start_time)");
  sqlRun("CREATE INDEX IF NOT EXISTS idx_reservations_legacy_local ON reservations(legacy_local_id)");
...
function initSchema() {
  ...
  CREATE INDEX IF NOT EXISTS idx_stock_manual_adj_item ON stock_manual_adjustments(menu_item_id);
  CREATE INDEX IF NOT EXISTS idx_stock_manual_adj_created ON stock_manual_adjustments(created_at);
`);
